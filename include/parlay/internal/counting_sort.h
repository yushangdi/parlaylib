...
sequence<size_t> seq_count_sort(slice<InIterator, InIterator> In,
                                ...
                                size_t num_buckets) {
  sequence<size_t> counts = sequence<size_t>::uninitialized(num_buckets + 1);
...
std::pair<sequence<size_t>, bool> count_sort_(slice<InIterator, InIterator> In,
  ...
  sequence<s_size_t> counts = sequence<s_size_t>::uninitialized(m);
  ...
  sequence<s_size_t> counts2 = sequence<s_size_t>::uninitialized(m);
...
auto count_sort(slice<InIterator, InIterator> In,
  ...
  if (n < max32 && num_buckets < max32)
    ...
    return count_sort_<uint32_t>(In, Out, Keys, num_buckets, parallelism,
                                 skip_if_in_one);
