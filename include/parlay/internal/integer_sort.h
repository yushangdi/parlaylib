...
sequence<Tint> get_counts(slice<Iterator,Iterator> In,
			  Get_Key const &g, size_t num_buckets) {
  size_t n = In.size();
  ...
  ends[g(In[n - 1])] = n;
  return tabulate(num_buckets, [&] (size_t i) -> Tint {
      return ends[i] - starts[i]; });
}

template <typename Tint = size_t, typename Iterator, typename Get_Key>
auto integer_sort_with_counts(slice<Iterator, Iterator> In,
			      Get_Key const &g,
			      size_t num_buckets) {
